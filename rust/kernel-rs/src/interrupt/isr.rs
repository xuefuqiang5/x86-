use crate::task::thread::{running_thread, MAGIC_NUM};
...
pub extern "C" fn general_program(vecnum: u32) {
    ...
    unsafe {
        use crate::vga::{put_char, put_int_hex, put_str, clear};
        clear();
...
pub fn init_idt_table() {
    ...
    for i in 0..IDT_DESC_CNT {
        unsafe { (*core::ptr::addr_of_mut!(idt_table))[i] = addr; }
    }
}

pub fn register_intr_handler(vecnum: u32, func: extern "C" fn()) {
    assert!((vecnum as usize) < IDT_DESC_CNT);
    unsafe { (*core::ptr::addr_of_mut!(idt_table))[vecnum as usize] = func as usize; }
}
