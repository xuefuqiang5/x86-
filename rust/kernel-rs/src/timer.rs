...
pub fn frequency_set(counter_port: u16, counter_no: u8, rwl: u8, counter_mode: u8, counter_value: u16) {
    unsafe {
        port_out8(PIT_CONTROL_PORT, counter_no << 6 | rwl << 4 | counter_mode << 1);
...
#[unsafe(no_mangle)]
pub extern "C" fn timer_init_c() { timer_init(); }
